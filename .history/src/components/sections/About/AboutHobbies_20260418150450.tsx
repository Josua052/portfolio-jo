    ...
    const draw = () => {
      ...
      balls.forEach((b) => {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          ctx.strokeStyle = "#f59e0b"; ctx.lineWidth = 0.9; ctx.stroke();
        }
        ctx.beginPath(); ctx.arc(0, 0, b.r * 0.3, 0, Math.PI * 2);
...
function BadmintonCanvas() {
  ...
  useEffect(() => {
    ...
    const shuttles = Array.from({ length: 18 }, () => ({
      ...
      vx: 0.5 + Math.random() * 0.8, vy: (Math.random() - 0.5) * 0.4,
      rot: Math.random() * Math.PI * 2, rotV: 0.01 + Math.random() * 0.02,
    ...
    const draw = () => {
      ...
      shuttles.forEach((s) => {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          ctx.strokeStyle = "#22c55e"; ctx.lineWidth = 0.8; ctx.stroke();
        }
        ctx.beginPath(); ctx.arc(0, 0, s.size, 0, Math.PI * 2);
        ctx.strokeStyle = "#22c55e"; ctx.lineWidth = 0.7; ctx.stroke();
        ctx.beginPath(); ctx.arc(0, 0, s.size * 0.25, 0, Math.PI * 2);
        ctx.strokeStyle = "#22c55e"; ctx.lineWidth = 1.2; ctx.stroke();
        ctx.restore();
      });
      raf = requestAnimationFrame(draw);
    };
...
function HikingCanvas() {
  ...
  useEffect(() => {
    ...
    const peakDefs = [
      [0,0.75],[0.07,0.4],[0.18,0.65],[0.28,0.28],
      [0.4,0.55],[0.52,0.22],[0.62,0.48],[0.74,0.3],[0.85,0.52],[0.94,0.35],[1,0.6],
    ];
    const draw = () => {
      const W = c.offsetWidth, H = c.offsetHeight;
      c.width = W; ctx.clearRect(0, 0, W, H);
      stars.forEach((s) => {
        ...
        ctx.fillStyle = `rgba(6,182,212,${s.alpha})`; ctx.fill();
      });
      ctx.beginPath(); ctx.moveTo(0, H);
      peakDefs.forEach(([px, py], i) => {
        if (i === 0) { ctx.lineTo(px * W, py * H); return; }
        const [ppx, ppy] = peakDefs[i - 1];
        ctx.quadraticCurveTo(ppx * W, ppy * H, ((ppx + px) / 2) * W, ((ppy + py) / 2) * H);
      });
      ctx.lineTo(W, H); ctx.closePath();
      ctx.strokeStyle = "rgba(6,182,212,0.12)"; ctx.lineWidth = 1.5; ctx.stroke();
      raf = requestAnimationFrame(draw);
    };
    draw();
    return () => cancelAnimationFrame(raf);
...
function TravellingCanvas() {
  ...
  useEffect(() => {
    ...
    const routes = nodes.map((_, i) => ({
      from: i, to: (i + 4 + Math.floor(Math.random() * 4)) % nodes.length,
    }));
    const travelers = routes.map((r) => ({
      ...r, prog: Math.random(), speed: 0.0015 + Math.random() * 0.003,
    }));
    const draw = () => {
      const W = c.offsetWidth, H = c.offsetHeight;
      c.width = W; ctx.clearRect(0, 0, W, H); t += 0.008;
      const pts = nodes.map((n) => ({ x: n.x * W, y: n.y * H, r: n.r, phase: n.phase }));
      routes.forEach((r) => {
        const a = pts[r.from], b = pts[r.to];
        ctx.beginPath(); ctx.setLineDash([2, 10]);
      ...
      pts.forEach((p) => {
        ...
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r + pulse * 2.5, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(99,102,241,${0.05 + pulse * 0.08})`; ctx.fill();
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r * (1.6 + pulse * 1.8), 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(99,102,241,${0.03 + pulse * 0.05})`; ctx.lineWidth = 0.8; ctx.stroke();
      });
      travelers.forEach((tr) => {
        tr.prog += tr.speed; if (tr.prog > 1) tr.prog = 0;
        const a = pts[tr.from], b = pts[tr.to];
        ctx.beginPath();
        ctx.arc(a.x + (b.x - a.x) * tr.prog, a.y + (b.y - a.y) * tr.prog, 2.5, 0, Math.PI * 2);
...
export default function AboutHobbies() {
  ...
  const goTo = (i: number) => {
    ...
    setVisible(false);
    setTimeout(() => { setActiveIdx(i); setVisible(true); }, 380);
  };
      ...
      <div className="container-custom hbs-inner">
        ...
        >
              ...
              {HOBBIES.map((h, i) => (
                ...
                >
                  <span
                    className="hbs-dot"
                    style={{
                      background: i === activeIdx ? meta.accent : "var(--border)",
          ...
          <div className="hbs-right" key={hobby.id}>
            ...
            {/* Corner label */}
            <span className="hbs-corner-label" style={{ color: meta.accent + "99" }}>
              {hobby.title}
            </span>
